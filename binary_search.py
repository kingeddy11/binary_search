#!/bin/python3
'''
JOKE: There are 2 hard problems in computer science: cache invalidation, nam
ing things, and off-by-1 errors.
    ...
    def go_big(left, right):
        ...
        if xs[mid] > x:
            ...
            if mid == left or xs[mid - 1] > x:
                ...
                right = mid - 1
            return go_big(left, right)
        return go_big(left, right)

    def go_small(left, right):
        """
        use binary search to find the lowest index with a value < x
        """
        if right == len(xs) - 1 & xs[right] == x:
            return right
        if left == right:
            if xs[left] == x:
                return left
        ...
        if xs[mid] > x:
            ...
            if mid == right or xs[mid + 1] < x:
                ...
                left = mid + 1
            return go_small(left, right)
        return go_small(left, right)

    first_pos = go_big(left, right)
    last_pos = go_small(left, right)
...
def argmin(f, lo, hi, epsilon=1e-3):
    '''
    Assumes that f is an input function that takes a float as input and retu
    rns a float with a unique global minimum,
    ...
    But the actual minimization code will all be a variant of this binary se
    arch.
    ...
    look like.
    Your output numbers are likely to be slightly different due to minor imp
    lementation details.
    Writing tests for code that uses floating point numbers is notoriously d
    ifficult.
